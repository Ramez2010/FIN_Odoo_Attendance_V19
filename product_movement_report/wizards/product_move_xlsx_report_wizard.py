...
class ProductMoveXlsx(models.TransientModel):
    ...
    def prepare_product_moves_xlsx_report(self):
        ...
        # التعديل الوحيد هنا: تحويل التواريخ لنصوص لضمان مرورها عبر JSON بدون مشاكل
        data = {
...
class ProductReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, *args):
        # --- بداية التعديل المطلوب (Fixing KeyError) ---
        # محاولة جلب السجل (الويزارد) من args كخطة بديلة
        wizard = args[0] if args else None

        # استخدام get لجلب البيانات بأمان، أو استخدام بيانات الويزارد في حالة الفشل
        start_date = data.get('start_date') or (str(wizard.start_date) if wizard else '')
        end_date = data.get('end_date') or (str(wizard.end_date) if wizard else '')
        product_name = data.get('product_name') or (wizard.product_id.name if wizard else '')
        ...
        # استخدام المتغير الآمن start_date
        sheet.merge_range('B6:C6', start_date, cell_format)

        sheet.write('D6', 'To:', cell_format)
        # استخدام المتغير الآمن end_date
        sheet.merge_range('E6:F6', end_date, cell_format)

        # استخدام المتغير الآمن product_name
        sheet.merge_range('A7:F7', product_name, cell_format)

        # استخدام المتغير الآمن moves
        sheet.add_table(
            f'A8:F{len(moves) + 9}', {
                'columns': [
                    {

from odoo import api, fields, models
from odoo.exceptions import ValidationError


class SaleOrder(models.Model):
    _inherit = 'sale.order'

    display_project_id = fields.Many2one('project.project', index=True, string='Project')

    analytic_account_id = fields.Many2one(
        comodel_name='account.analytic.account',
        string="Analytic Account",
        copy=False,
        check_company=True,
        compute='_compute_analytic_account_id',
        store=True,
        readonly=False
    )

    employee = fields.Many2one('hr.employee')

    def action_confirm(self):
        for order in self:
            if not order.analytic_account_id:
                raise ValidationError("The Analytic Account Field Must Be Set")
        return super(SaleOrder, self).action_confirm()

    @api.depends('display_project_id')
    def _compute_analytic_account_id(self):
        for rec in self:
            if rec.display_project_id:
                if hasattr(rec.display_project_id, 'account_id'):
                    rec.analytic_account_id = rec.display_project_id.account_id
                elif hasattr(rec.display_project_id, 'analytic_account_id'):
                    rec.analytic_account_id = rec.display_project_id.analytic_account_id
                else:
                    rec.analytic_account_id = False
            else:
                rec.analytic_account_id = False

    @api.model
    def sale_project_update(self):
        orders = self.search([('analytic_account_id', '!=', False), ('display_project_id', '=', False)])
        for order in orders:
            project = self.env['project.project'].search([
                ('account_id', '=', order.analytic_account_id.id)
            ], limit=1)

            if project:
                order.display_project_id = project.id